...
3. **Run create script**:
   ```bash
   ${SPECKIT_ROOT}/scripts/create-new-feature.sh --json --number N --short-name "short-name" "$ARGUMENTS"
   ```
...
**Tasks:**
1. Run `${SPECKIT_ROOT}/scripts/check-prerequisites.sh --json` and parse output
...
3. Generate `tasks.md` using `${SPECKIT_ROOT}/assets/templates/tasks-template.md` structure:
   - Phase 1: Setup tasks
