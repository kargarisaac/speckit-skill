...
3. **Execute plan workflow**: Follow the structure in IMPL_PLAN template to:
   ...
   - Evaluate any explicit constraints in the spec (ERROR if violations unjustified)
   - Phase 0: Generate research.md (resolve all NEEDS CLARIFICATION)
   - Phase 0: Generate decisions.md (capture ADRs from research decisions)
   - Phase 1: Generate data-model.md, contracts/, quickstart.md
   - Phase 1: Update agent context by running the agent script (no file creation)
...
3. **Agent context update**:
   - Run `${SPECKIT_ROOT}/scripts/update-agent-context.sh [agent_type]` (e.g., `gemini`, `cursor-agent`, `windsurf`)
   - These scripts detect which AI agent is in use
   - Update the appropriate agent-specific context file
   - Add only new technology from current plan
   - Preserve manual additions between markers

**Output**: data-model.md, /contracts/*, quickstart.md, agent-specific file (if present)
